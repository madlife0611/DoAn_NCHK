...
trait SuppliersModel
{
	...
	//tinh tong sp da cung cap
	public function modelTotalProducts($mancc)
	{
		//lay bien ket noi csdl
		$db = Connection::getInstance();
		//thuc hien truy van
		$query = $db->query("select sum(soluong) from products where mancc = $mancc");
		//tra ve so luong ban ghi
		return $query->fetchColumn();
	}
	//lấy tổng số vật tư sắp đến hạn bảo trì trong 14 ngày
	public function modelTotalMaintenanceProductsOn14Days($mancc)
	{
		//lay bien ket noi csdl
		$conn = Connection::getInstance();
		//thuc hien truy van
		$query = $conn->query("select * from products where DATEDIFF(hanbaotri,now())<14 and mancc = $mancc");
		return $query->rowCount();
	}
	//lấy tổng số vật tư đang hạn bảo trì
	public function modelTotalMaintenance($mancc)
	{
		//lay bien ket noi csdl
		$conn = Connection::getInstance();
		//thuc hien truy van
		$query = $conn->query("select * from maintenances where trangthai = 0 and mancc = $mancc");
		return $query->rowCount();
	}
